...
for row_index in range(sheet0.nrows):
    ...
    boxtag_sect = soup.select('div[class^="kno-xs"]')
        
    if boxtag_sect:
        for i in range(0, len(boxtag_sect)):
            array1 = boxtag_sect[i]
            sect_title = array1.span.string.encode('utf8')
            cnt = array1.find_all(class_="vrt kno-fb-ctx")                       

            text = "이것을 찾으셨나요?".encode('utf8')
